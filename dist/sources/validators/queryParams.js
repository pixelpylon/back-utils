...
const isFilter = v.props({
  field: v.isFilled,
  value: v.some(v.isString, v.isBoolean, v.isNumber, v.items(v.isString), v.items(v.isBoolean), v.items(v.isNumber)),
  op: v.optional(v.isFilled),
})

const isFilters = v.optional(v.some(v.isNull, v.items(isFilter)))

const isOrdering = v.optional(
  v.some(
    v.isNull,
    v.items(v.isFilled),
    v.items(
...
const paginatedListParams = v.props({
  ...
  filters: isFilters,
  oredering: isOrdering,
  cursor: v.optional(v.some(v.isNull, v.isFilled)),
})
